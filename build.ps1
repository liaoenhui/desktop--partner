param([string]$OutputName = 'SilverWolfPet-v2.6.18-preview')
...
& (Join-Path $frameworkRoot 'csc.exe') /nologo /target:winexe /platform:x64 /optimize+ /codepage:65001 ('/win32icon:' + (Join-Path $projectRoot 'assets\silver-wolf.ico')) ('/win32manifest:' + (Join-Path $projectRoot 'src\app.manifest')) ('/out:' + (Join-Path $outputPath 'SilverWolfPet.exe')) $references (Join-Path $projectRoot 'src\Pet.cs') (Join-Path $projectRoot 'src\CodexMonitor.cs') (Join-Path $projectRoot 'src\Companion.cs') (Join-Path $projectRoot 'src\Touch.cs') (Join-Path $projectRoot 'src\Experience.cs') (Join-Path $projectRoot 'src\Sidebar.cs') (Join-Path $projectRoot 'src\CodexForm.cs')

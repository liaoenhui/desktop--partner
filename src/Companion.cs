...
public partial class PetWindow {
    ...
    void ApplyBubbleTheme() {
        bool work=codexFormWorking;if(bubbleThemeWork.HasValue&&bubbleThemeWork.Value==work)return;bubbleThemeWork=work;
        if(work) {
    ...
    void CompanionTick() {
        ...
        if(animate&&(state=="idle"||state=="blink"||(inputAnimating&&state=="game"))) {
            ...
            // Keep it centered on the character and above the follower quota badge.
            keyboard.Margin=KeyboardPlacement(prefs.Size);
